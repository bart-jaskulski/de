...
static int bw, bh, bx, by;
static int bu; // Underline height
...
init ()
{
    ...
    char *font_list_str = xstrdup(font_names);
    char *output_list_str = xstrdup(outputs);
    char *saveptr_font, *saveptr_output, *font_token, *output_token;

    // Geometry parsing from config.h
    if (!parse_geometry_string((char*)bar_geometry, geom_v)) {
        fprintf(stderr, "Invalid geometry string in config.h\n");
        exit(EXIT_FAILURE);
    }
    bw = geom_v[0];
    bh = geom_v[1];
    ...
    by = geom_v[3];
    bu = underline_height;
    dbgc = bgc = parse_color(background_color, NULL, BLACK);
    dfgc = fgc = parse_color(foreground_color, NULL, WHITE);
    dugc = ugc = parse_color(underline_color, NULL, fgc);

    // Font loading from config.h
    for (font_token = strtok_r(font_list_str, ",", &saveptr_font);
         font_token != NULL;
         font_token = strtok_r(NULL, ",", &saveptr_font)) {
        font_load(font_token);
    }
    free(font_list_str);


    // Output names parsing from config.h
    num_outputs = 0;
    if (output_list_str && output_list_str[0] != '\0') {
        for (output_token = strtok_r(output_list_str, ",", &saveptr_output);
             output_token != NULL;
             output_token = strtok_r(NULL, ",", &saveptr_output)) {
            output_names = xreallocarray(output_names, num_outputs + 1, sizeof(char*));
            output_names[num_outputs++] = xstrdup(output_token);
        }
    }
    free(output_list_str);


    // Try to load a default font if none were loaded from config
    if (font_count == 0)
        font_load("fixed");
